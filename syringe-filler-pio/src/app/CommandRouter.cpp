  ...
  void handleSerial()
  {
    while (Serial.available())
    {
      ...
      if (c == '\n')
      {
        ...
        { // free-run style speed setter (compat)
        ...
        else if (input.startsWith("movemm "))
        { // relative in mm
        ...
        { // absolute in steps
          ...
          if (tgt < Pins::MIN_POS_STEPS)
            tgt = Pins::MIN_POS_STEPS;
          if (tgt > Pins::MAX_POS_STEPS)
            tgt = Pins::MAX_POS_STEPS;
          Axis::enable(true);
          Axis::moveTo(tgt);
          Axis::enable(false);
          Serial.println("OK");
        }
        else if (input.startsWith("gotomm "))
        { // absolute in mm
          float mm = input.substring(7).toFloat();
        ...
        else if (input.startsWith("base "))
        {
          ...
          {
            Serial.print("Usage: base <0..");
            Serial.print(Pins::NUM_BASES);
            Serial.println(">");
          }
          else
          {
            Serial.print("[Base] selected=");
            Serial.println(Bases::selected());
          }
        }
        ...
        else if (input.startsWith("servopulse"))
        {
          ...
          if (firstSpace >= 0)
          {
            ...
            if (sp > 0)
            {
              ...
              Serial.println("Usage: servopulse <channel> <us>");
          }
          else
            Serial.println("Usage: servopulse <channel> <us>");
        }
        ...
        else if (input.startsWith("potmove "))
        {
          ...
          else
          {
            ...
            if (ok)
              Serial.println("OK potmove");
            else
              Serial.println("ERR potmove");
          }
        }
        else
        {
